...
void Question::renderRewardMessage(int x, int y, const std::string &fontFile, SDL_Color color, int fontSize, SDL_Renderer *renderer, int width){
    ...
    //We need to first render to a surface as that's what TTF_RenderText
    //returns, then load that surface into a texture
    SDL_Surface *surf = TTF_RenderText_Blended_Wrapped(font, (privRewardMessage + "\n\nPress ENTER to continue...").c_str(), color, width);
    if (surf == nullptr){
        TTF_CloseFont(font);
        std::cout << "TTF_RenderText" << " Error: " << SDL_GetError() << std::endl;
        return;
...
bool Question::checkAnswer(std::string answer){
    if (privAnswer.compare(answer) == 0){
        return true;
    }else{
        return false;
    }
